      ...
      <div className="w-full max-w-[15rem] px-4">
        <label className="w-full text-gray-5 overflow-hidden">
          <div className="relative">
            <MdDateRange
              className={`absolute left-4 top-2.5 h-6 w-6 text-gray-5 ${classPrefix}`}
            />
            <DatePicker
              selected={start}
              ...
              showMonthDropdown
              showYearDropdown
              className={`text-sm lg:text-md w-full text-gray-5 rounded-lg border border-stroke bg-transparent py-3 pl-12 pr-6 outline-none focus:border-primary focus-visible:shadow-none ${classInput}`}
            />
          </div>
        </label>
      </div>
