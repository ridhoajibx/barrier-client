...
export default function FormCloseGate(props: Props) {
  ...
  const onSubmit: SubmitHandler<FormValues> = async (value) => {
    ...
    setLoading(true);
    try {
      const response = await axios.post(`gate/close`, newData, config);
    ...
    <Fragment>
      <ModalHeader
        ...
        <div className="w-full flex flex-col gap-1 px-2">
          <h3 className="text-lg font-semibold">Gate Force Close</h3>
      ...
      <div className="w-full">
        ...
        <div className="w-full flex gap-2 justify-end p-4 border-t-2 border-gray">
          <Button
            type="submit"
            variant=""
            className="w-full text-white rounded-md text-sm shadow-card border-green-500 bg-green-500 duration-300 active:scale-90"
            onClick={handleSubmit(onSubmit)}
            disabled={pending || !isValid}>
            <span className="font-semibold">Close Gate</span>
            {pending ? (
