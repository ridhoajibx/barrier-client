import React, {
  ...
  import { Filter, fuzzyFilter } from "../components";
      ...
      <div className="grid grid-cols-1">
        <div className="col-span-1 p-4 overflow-x-auto">
          <table className="w-full table-auto overflow-hidden rounded-xl shadow-md">
            ...
            <thead
              className={`divide-y divide-gray-4 text-xs text-graydark tracking-wide ${
            ...
            <tbody
              className={`divide-y divide-gray text-graydark text-xs`}>
              {table.getRowModel().rows.map((row) => {
                let ganjil = row?.index % 2 == 1;
                return (
                  <tr key={row.id} className={`${ganjil ? "bg-gray-2" : ""}`}>
